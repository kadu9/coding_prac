...
dayGroupedHosts = dayToHostPairTuple.groupByKey().mapValues(list)
...
print 'host count %s' % dailyHosts.count()
...
sortedByDay = groupedByDay.map(lambda (x,y) : (x, len(y)/len(set(y))))
...
errByDate = errDateSorted.takeOrdered(22, lambda s: 1 * s[0])
...
errHourList = hourRecordsSorted.takeOrdered(24, lambda s: 1 * s[0])
